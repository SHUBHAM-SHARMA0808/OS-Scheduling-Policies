...
{   
    ...
    if(choice == 1)
    {
    	...
    	cout<<"Enter arrival_time, burst_time, priority"<<endl;
	...
	for(int current_time = 0 ; ;)
	{
	    ...
	    if(flag == 1)
		{
	        ...
	        for(int i = 0 ; i<no_of_processes ; i++)
	        {		     
			    if(arrival_time[i]<=current_time && !processed[i] && i!=run_process)
			    {
				    d_wait_time[i]++;
				    if(d_wait_time[i]%threshold_wait == 0)
					    priority[i]--;
			    }
			    cout<<d_wait_time[i]<<" ";       		
		    }
